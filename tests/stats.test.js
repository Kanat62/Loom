...
test('stats: getUsageByRole агрегирует по агенту, checker (без usage-событий) не попадает в таблицу', async () => {
  ...
  try {
    ...
    journal.logEvent({ project_id: project.id, type: 'usage', agent: 'architect', tokens_in: 500, tokens_out: 300, cache_read: 0, cost_usd: 0.05, duration_ms: 3000 });
    ...
    assert.equal(byAgent.architect.calls, 1);
...
test('stats: formatSpendReport сходится - сумма по ролям равна ИТОГО, календарное время >= времени работы', async () => {
  ...
  try {
    ...
    journal.logEvent({ project_id: project.id, type: 'usage', agent: 'architect', tokens_in: 200, tokens_out: 100, cache_read: 0, cost_usd: 2.5, duration_ms: 8000 });
